...
app.whenReady().then(async () => {
  ...
  // 会话模型网络守卫(v0.15.5):Anthropic 协议(含 Kuro)经本地代理做模型白名单;
  // 失败时守卫不接管,PreToolUse 白名单仍生效。
  try { await modelGuard.start(); } catch (e) { console.error('[model-guard] 启动失败:', e.message); }
