...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.19';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.19.exe',
  'DeskTopUI Setup 0.9.19.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    ...
    '',
    '- **垂直居中**:消息缩略导航默认居于消息区垂直中部',
    '- **边缘淡出省略**:消息太多装不下时,导航上下边缘淡出,表示两侧还有省略内容',
    '- **悬停项更醒目**:悬停中心项放大到 1.15x 并高亮(主题色描边/底色),上下两侧项更快退回小横杠、远处项压暗衬托',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.19', body: notes, draft: false, prerelease: false });
