...
async function main() {
  ...
  if (!/^version: 0\.15\.19\s*$/m.test(yml) || !yml.includes('url: ' + assets[0].remote)) throw new Error('Update metadata mismatch');
  ...
  // Exercise the exact version-check function used by the installed 0.15.18 UI.
  const Module = require('module');
  const originalLoad = Module._load;
  Module._load = function (request, ...rest) {
    if (request === 'electron') return { app: { getVersion: () => '0.15.18', isPackaged: true } };
    return originalLoad.call(this, request, ...rest);
  };
  try {
    const result = await require('../src/main/updater').checkRepoVersion();
    if (result.latest !== '0.15.19' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.19: ' + JSON.stringify(result));
