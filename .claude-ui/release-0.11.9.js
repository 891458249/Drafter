...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.11.9';
...
const ASSETS = [
  { local: 'Drafter Setup 0.11.9.exe', remote: 'Drafter-Setup-0.11.9.exe' },
  { local: 'Drafter Setup 0.11.9.exe.blockmap', remote: 'Drafter-Setup-0.11.9.exe.blockmap' },
...
(async () => {
  ...
  const notes = [
    '### v0.11.9(2026-08-26):修复插件列表停用后模型设置报 HTTP 404,根载体不可再被停用',
    '',
    '**问题**:在插件列表把第一张卡片「include」停用后,Harness 板块大面积报错',
    '(如设置 → 模型:加载提供方目录失败 transport failure for /api/llm.providers: HTTP 404),',
    '且重启后仍显示已停用。',
    '',
    '**根因**:include 是 harness 插件树的**根载体**——插件列表里的所有插件都挂在它下面。',
    '运行态停用它等价于拆掉整棵树:API 网关随之卸载,/api 全部 404。',
    '(启停开关是 v0.11.8 新增的,未对根载体做防护。)',
    '',
    '**修复**:',
    '- 根载体(include)与插件组的启停请求被宿主拒绝,并给出明确提示;',
    '- 插件列表中这类载体卡片不再显示启停按钮;',
    '- 启停补丁层里残留的 include 行在启动时自动剔除修复(本次事故产生的坏状态会自愈)。',
    '',
    '**验证**:冒烟 18 项断言(含开关被拒后 API 仍健康、补丁层修复、重启保持);',
    '真实 UI 点击全链路;npm test 152/152。重启 App 生效(electron-updater 可自动更新)。',
  ].join('\n');
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'Drafter v0.11.9', body: notes, draft: false, prerelease: false });
