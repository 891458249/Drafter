...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.33';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.33.exe',
  'DeskTopUI Setup 0.9.33.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.33(2026-08-12):链接外抛浏览器 + Chat 面板 + 窄窗防乱版',
    '',
    '- **修复点击网页链接主窗口变浏览器**:会话中的 http(s) 链接一律改为系统默认浏览器打开(will-navigate 拦截 + window.open 拦截),App 窗口不再被劫持、不再无法返回',
    '- **Chat 板块开放右侧面板**:变更/编辑器/预览/任务/终端五个 tab 在纯对话模式同样可用,点文件链接直接预览编辑(配合 v0.9.32 的路径可点击)',
    '- **窗口过窄 composer 不再乱版**:工具栏允许换行、禁止压缩按钮,中文按钮不再被挤成竖排',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.33', body: notes, draft: false, prerelease: false });
