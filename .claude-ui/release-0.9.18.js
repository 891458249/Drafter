...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.18';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.18.exe',
  'DeskTopUI Setup 0.9.18.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.18(2026-08-10):消息导航悬停展开修复——上下缩放严格对称',
    '',
    '- 修复消息缩略导航悬停时「上下缩放不平均」:改为槽位+transform 结构,悬停放大不再推挤邻近项,光标上下两侧按距离对称衰减,也无抖动',
    '- 默认形态仍是一条条小横杠,悬停由小到大展开完整摘要',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.18', body: notes, draft: false, prerelease: false });
