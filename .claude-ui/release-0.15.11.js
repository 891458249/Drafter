...
const TAG = 'v0.15.11';
const DIST = path.join(ROOT, 'dist/release-0.15.11');
const names = [
  ['Drafter Setup 0.15.11.exe', 'Drafter-Setup-0.15.11.exe'],
  ['Drafter Setup 0.15.11.exe.blockmap', 'Drafter-Setup-0.15.11.exe.blockmap'],
  ['latest.yml', 'latest.yml'],
];
const NOTES = `修复:视频附件加入后无法被 AI 识别分析——发送时只注入文件名/大小,AI 实际「看不到」视频内容。

- 根因:辅助模型分析对视频附件硬编码拒绝,无论是否配置「视频辅助」模型都只注入元信息兜底。
- 修复:视频改走 chat/completions 的 video_url 块(Qwen-VL / GLM-4V / Gemini-OpenAI-兼容等多模态网关支持,≤20MB,mp4/mov/webm),配中文视频分析 prompt;网关不支持时自动落回元信息兜底,不会报错。
- 提示:需在「API Keys 设置 → 视频辅助」选一个支持视频的多模态模型才会真正分析视频内容;3D 模型附件仍仅注入文件信息。

验证:367 项测试通过(新增 video_url 块结构、mov/webm MIME 映射、injectMedia 视频端到端用例)。
...
async function main() {
  ...
  if (!/^version: 0\.15\.11\s*$/m.test(yml) || !yml.includes('url: ' + assets[0].remote)) throw new Error('Update metadata mismatch');
  ...
  // Exercise the exact version-check function used by the installed 0.15.10 UI.
  const Module = require('module');
  const originalLoad = Module._load;
  Module._load = function (request, ...rest) {
    if (request === 'electron') return { app: { getVersion: () => '0.15.10', isPackaged: true } };
    return originalLoad.call(this, request, ...rest);
  };
  try {
    const result = await require('../src/main/updater').checkRepoVersion();
    if (result.latest !== '0.15.11' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.11: ' + JSON.stringify(result));
