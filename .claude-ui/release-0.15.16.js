...
const TAG = 'v0.15.16';
const DIST = path.join(ROOT, 'dist/release-0.15.16');
const names = [
  ['Drafter Setup 0.15.16.exe', 'Drafter-Setup-0.15.16.exe'],
  ['Drafter Setup 0.15.16.exe.blockmap', 'Drafter-Setup-0.15.16.exe.blockmap'],
  ['latest.yml', 'latest.yml'],
];
const NOTES = `新板块「扩展」(顶栏 Chat 与「创作」之间):Skill 技能与自定义子 Agent 的创建/管理。

- Skill 技能:会话挂载(composer「🧩 技能」)后走渐进式披露——systemPrompt 只放名称+描述索引,模型需要时调用进程内 MCP 工具 use_skill 取回完整指令;发送前还可在菜单第二级「本条消息指定」(★)手动勾选,全文随该条消息注入、优先级最高,发送后自动清空(极速问答模式同样生效)。
- 自定义子 Agent:按作用域挂载——全局 / 指定项目 / 指定会话;会话作用域可在 composer「🤖 子 Agent」菜单底部「自定义 Agent」分组按需勾选。支持固定模型(下拉按启用 Key 的 chat 模型列出)与工具白名单,委派时经守卫校验,禁止嵌套委派。
- 编辑器复用 Gem 三栏弹窗;「✨ AI 起草」一句话生成技能指令/Agent 提示词;内置 3+3 预置模板(代码审查清单、Commit 信息助手、文档撰写 / code-reviewer、test-engineer、code-explorer),只读可复制副本;支持导入/导出 Claude Code 原生 frontmatter .md 格式。
- 数据存于 Drafter 本地设置(settings.extensions),不读写 ~/.claude 目录。

验证:421 项测试通过(新增 18 例);端到端冒烟(隔离实例 + 本地假网关当真 Key,真实 SDK 发送)确认技能索引与 pinned 全文注入、会话挂载落库、自定义 Agent 挂载落库。
...
async function main() {
  ...
  // Exercise the exact version-check function used by the installed 0.15.15 UI.
  const Module = require('module');
  const originalLoad = Module._load;
  Module._load = function (request, ...rest) {
    if (request === 'electron') return { app: { getVersion: () => '0.15.15', isPackaged: true } };
    return originalLoad.call(this, request, ...rest);
  };
  try {
    const result = await require('../src/main/updater').checkRepoVersion();
    if (result.latest !== '0.15.16' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.16: ' + JSON.stringify(result));
