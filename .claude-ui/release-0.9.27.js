...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.27';
...
(async () => {
  ...
  const notes = [
    '### v0.9.27(2026-08-10):消息导航密度自适应 + 边界渐出去遮罩化',
    '',
    '- 修复「消息过多时导航栏显示不正确」:导航项按消息数量自动等比压缩槽距与横杠厚度,无论多少条消息都全部可见,macOS Dock 式悬停放大在任何数量下都可用(仅极端数量才退回可滚动小窗)',
    '- 导航栏上下边缘的渐出效果不再给整栏加渐变遮罩,改为靠近边缘的消息按键自身透明度逐渐变淡',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.27', body: notes, draft: false, prerelease: false });
