...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.21';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.21.exe',
  'DeskTopUI Setup 0.9.21.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    ...
    '',
    '- 消息多到装不下时,缩略导航不再占满整条,收成居中的小窗(约为会话区 1/3 高)',
    '- **悬停位置即总列表相应位置**:鼠标在小窗内移动时,列表按光标比例自动滚动到整体的对应位置,指哪看哪;上下淡出保留省略提示',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.21', body: notes, draft: false, prerelease: false });
