...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.23';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.23.exe',
  'DeskTopUI Setup 0.9.23.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.23(2026-08-10):回撤 v0.9.22 + 边界淡出优化',
    '',
    '- 回撤 v0.9.22 的整条右缘悬停方案,恢复 v0.9.21 的居中小窗 + 悬停比例代理滚动',
    '- **边缘淡出更合理**:滚动未探到边界的一侧保持淡出,一旦滚到顶/底,该侧淡出即消失',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.23', body: notes, draft: false, prerelease: false });
