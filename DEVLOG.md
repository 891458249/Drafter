...
- **Bifrost 节点外观**(参照 Maya Bifrost 图编辑器):深色石板卡(#3d4348)+ 分类色顶部色条(3px,头部本体保持深色)+ 小方块端口(8×11,左蓝输入/右绿输出,带端口名,半嵌两侧边缘不遮挡内容);修全局 `.input` 样式泄漏把输入端口撑成 28px 的问题(padding 透传)。
- **双向连线**:左右两侧引脚大小一致,左引脚可主动拉到其他节点右引脚(反向拉起,复用类型槽校验);此前只有右→左方向。
- **悬浮工具栏**:拖拽移动节点时自动隐藏,停手后重新跟随显示;画布平移/缩放/节点移动时同步跟随。
- **形状与颜色**:颜色条默认收起,点「形状」按钮展开颜色条+形状下拉菜单(方/圆/卡三选,不再是点一次变一次);颜色改为改变**整个节点**配色(CSS 变量整体着色+头部色条),不再只改一条边;形状/颜色随画布持久化并在重开时恢复。
- 冒烟(CDP,干净实例):工具栏拖拽隐藏/跟随、Bifrost 外观(色条/深色/端口尺寸/端口名)、形状按钮展开与整节点改色、左引脚反向连线(1 条)、Delete、零渲染错误;npm test 190/190。
...
### v0.15.6 — 修「SDK 实报窗口仍是错的」:claude.exe 对网关模型一律报默认 200k
...
- **订正 v0.15.4 调研错误**:Kimi K3 实为 **1M**(1,048,576,2026-07 发布,KDA 混合线性注意力架构;注意部分第三方宿主截断至 205K/262K,官方端才是 1M)——用户指出后复查官方资料确认;K2.x 系(含 kimi-for-coding)才是 256K。顺带:DeepSeek V4 系 = 1M
...
- 效果:Kimi K3 分母 200k→1M;kimi-for-coding→256K;Claude 新代(Opus 4.6+/Sonnet 5)即使被 claude.exe 误报 200k 也纠正为 1M(GA)
- 重要边界(已与用户说明):上下文窗口是提供方服务端硬上限,本地数据再多也不能突破窗口本身;要 1M 须用本身支持 1M 的模型(K3 恰好就是);「本地数据超窗口利用」的正解是 RAG(检索注入),属独立功能
- 测试:ctxwin.test.js 补 K3=1M/lookup/effective 用例,context-window.test.js 补 3 例(网关纠正/Claude 1M 纠正/主模型优先);npm test 318/318
- 版本注:并行会话已先行发布 v0.15.5(hard-block unchecked subagent models),故本批顺延为 v0.15.6
