...
const TAG = 'v0.15.8';
const DIST = path.join(ROOT, 'dist/release-0.15.8');
const names = [
  ['Drafter Setup 0.15.8.exe', 'Drafter-Setup-0.15.8.exe'],
  ['Drafter Setup 0.15.8.exe.blockmap', 'Drafter-Setup-0.15.8.exe.blockmap'],
  ['latest.yml', 'latest.yml'],
];
const NOTES = `修复按住鼠标选择文字时把上方所有文字连带选中的问题。

- 根因：全局未设 user-select。从消息气泡向上拖拽越过消息滚动容器上边界时，Chromium 原生选择把焦点跳到顶栏等可选 UI 元素，从顶栏到按下点的所有文字（含 UI 按钮/标签）被一并选中。
- 修复：UI 骨架（顶栏/侧栏/右栏/输入区/消息角色标签/导航条/弹窗遮罩/代码块头部/思考块/权限卡/状态行等）设 user-select:none；消息正文、代码块、输入框、编辑器等文字交互区显式 user-select:text。

验证：356 项测试通过；CDP 探针对打包版实测，修复后选区锚点/焦点均停留在按下的消息内，不再逃逸到顶栏。
...
async function main() {
  ...
  try {
    ...
    if (result.latest !== '0.15.8' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.8: ' + JSON.stringify(result));
