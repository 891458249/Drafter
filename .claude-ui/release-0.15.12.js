...
const TAG = 'v0.15.12';
const DIST = path.join(ROOT, 'dist/release-0.15.12');
const names = [
  ['Drafter Setup 0.15.12.exe', 'Drafter-Setup-0.15.12.exe'],
  ['Drafter Setup 0.15.12.exe.blockmap', 'Drafter-Setup-0.15.12.exe.blockmap'],
  ['latest.yml', 'latest.yml'],
];
const NOTES = `修复:配置了 gpt-6-astra 等有视频能力的模型,视频附件仍分析失败(HTTP 400 \`Invalid value: 'video_url'\`)。

- 根因(实测确认):Kuro 等 OpenAI Responses 风格网关没有任何视频字节入参——\`video_url\` 判为非法值、\`input_file\` 仅支持文档类 MIME;v0.15.11 的 video_url 直发在这类网关必然 400。
- 修复:改为本地抽帧走图像通道——用内置 Chromium 解码本地视频(≤64MB,零新增依赖),按时间轴均匀抽 6 帧作为多个 image_url 块 + 时序关键帧 prompt 发送(OpenAI 式视频理解标准做法);失败自动回退 video_url(兼容 Qwen-VL / GLM-4V 原生视频网关),再失败回退元信息兜底,不会报错。
- 提示:仍需在「API Keys 设置 → 视频辅助」选一个多模态模型才会真正分析视频内容。

验证:368 项测试通过;真机实测(Kuro gpt-6-astra + 10.9MB mp4)抽帧 864ms、网关返回完整中文视频内容描述。
...
async function main() {
  ...
  if (!/^version: 0\.15\.12\s*$/m.test(yml) || !yml.includes('url: ' + assets[0].remote)) throw new Error('Update metadata mismatch');
  ...
  // Exercise the exact version-check function used by the installed 0.15.11 UI.
  const Module = require('module');
  const originalLoad = Module._load;
  Module._load = function (request, ...rest) {
    if (request === 'electron') return { app: { getVersion: () => '0.15.11', isPackaged: true } };
    return originalLoad.call(this, request, ...rest);
  };
  try {
    const result = await require('../src/main/updater').checkRepoVersion();
    if (result.latest !== '0.15.12' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.12: ' + JSON.stringify(result));
