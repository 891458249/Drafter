...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.26';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.26.exe',
  'DeskTopUI Setup 0.9.26.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.26(2026-08-10):Gem 菜单不再被窗口底边裁切',
    '',
    '- 修复 Gem 选择器菜单在窗口较矮时被底边裁掉的问题:下方空间不足时自动向上展开,并按可用空间收高度',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.26', body: notes, draft: false, prerelease: false });
