...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.28';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.28.exe',
  'DeskTopUI Setup 0.9.28.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    ...
    '',
    '- 附件不再限制文件大小(图片去掉 5MB 上限;文本附件本就只按路径引用)',
    '- 双击消息里的 📄 附件卡片(或输入框待发区的附件),直接在右侧编辑器面板查看内容(上限放宽到 20MB)',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.28', body: notes, draft: false, prerelease: false });
