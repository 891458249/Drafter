...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.24';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.24.exe',
  'DeskTopUI Setup 0.9.24.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.24(2026-08-10):导航窗口边缘项完整显示',
    '',
    '- 修复消息缩略导航窗口上下边缘的横杠只显示半条:悬停代理滚动按项距取整吸附,边缘项要么完整显示、要么完整隐入淡出区',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.24', body: notes, draft: false, prerelease: false });
