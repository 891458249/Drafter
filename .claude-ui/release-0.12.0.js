...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.12.0';
...
const ASSETS = [
  { local: 'Drafter Setup 0.12.0.exe', remote: 'Drafter-Setup-0.12.0.exe' },
  { local: 'Drafter Setup 0.12.0.exe.blockmap', remote: 'Drafter-Setup-0.12.0.exe.blockmap' },
...
(async () => {
  ...
  const notes = [
    '### v0.12.0(2026-08-31):修复 Harness 首条对话 PI_AI_ERROR（缺 Anthropic SDK）与 Kimi 404',
    '',
    '**问题**:Harness 首次发送消息后立即显示「本轮运行失败」，错误为',
    '`Cannot find package @anthropic-ai/sdk imported from ...pi-ai...anthropic-messages.js`。',
    '补齐该包后，Kimi Coding 又返回 404。',
    '',
    '**修复**:',
    '- 补齐 pi-ai 的 Anthropic / OpenAI SDK 及完整运行时依赖闭包到打包目录；',
    '- 模块解析支持只有子路径 exports、没有根入口的依赖包；',
    '- Kimi Coding (`api.kimi.com/coding/v1`) 改走 OpenAI Chat Completions 协议，',
    '  避免 Anthropic SDK 固定追加 `/v1/messages` 形成错误的 `/coding/v1/v1/messages` 地址。',
    '',
    '**验证**:npm test 153/153；打包态使用真实 Kimi Key 完成一次消息往返，助手回复成功。',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'Drafter v0.12.0', body: notes, draft: false, prerelease: false });
