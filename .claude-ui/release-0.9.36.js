...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.36';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'Drafter Setup 0.9.36.exe',
  'Drafter Setup 0.9.36.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.36(2026-08-12):系统通知可点击跳转会话',
    '',
    '- **任务完成/出错与权限确认的系统通知(toast)支持点击**:点击后自动唤出 App 窗口(托盘/最小化均可)并直接跳转到对应会话,跨板块会话自动跟随切换',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'Drafter v0.9.36', body: notes, draft: false, prerelease: false });
