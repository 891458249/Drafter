...
const TAG = 'v0.15.10';
const DIST = path.join(ROOT, 'dist/release-0.15.10');
const names = [
  ['Drafter Setup 0.15.10.exe', 'Drafter-Setup-0.15.10.exe'],
  ['Drafter Setup 0.15.10.exe.blockmap', 'Drafter-Setup-0.15.10.exe.blockmap'],
  ['latest.yml', 'latest.yml'],
];
const NOTES = `拆分子任务改为发送按钮旁的「激活/关闭」开关。

- 「⧉」开关移到输入框发送按钮左侧;点击激活(高亮)后,你发出的下一条消息不直接发送,而是由当前会话的 AI 把这条需求拆成多个子任务。
- 弹出可编辑的确认卡片:可改标题/说明、增删条目;确认后为每个子任务新建一个并行 code 会话(继承当前会话的项目/模型/Key/权限/推理深度/子 Agent/Gem),标题前缀 ⧉,各自并行运行,不打断当前视图。
- 再次点击开关即关闭,恢复正常发送;拆分失败时仍弹出卡片显示错误,可手动编辑后确认或取消。

验证:364 项测试通过;端到端冒烟(本地假 OpenAI 服务 + 真 Key)实测「激活开关→发送→真实拆分→可编辑卡片→确认→并行会话创建」全链路。
...
async function main() {
  ...
  if (!/^version: 0\.15\.10\s*$/m.test(yml) || !yml.includes('url: ' + assets[0].remote)) throw new Error('Update metadata mismatch');
  ...
  // Exercise the exact version-check function used by the installed 0.15.9 UI.
  const Module = require('module');
  const originalLoad = Module._load;
  Module._load = function (request, ...rest) {
    if (request === 'electron') return { app: { getVersion: () => '0.15.9', isPackaged: true } };
    return originalLoad.call(this, request, ...rest);
  };
  try {
    const result = await require('../src/main/updater').checkRepoVersion();
    if (result.latest !== '0.15.10' || !result.hasUpdate) throw new Error('App update check did not detect 0.15.10: ' + JSON.stringify(result));
