...
export class TestWorkspaces implements IWorkspaces {
   ...
   * production face's observable effect: the id leaves the list state
   * entirely (workspace accounting slot and archive set alike).
  ...
  async deleteSession(sessionId: SessionId): Promise<void> {
    ...
    await this.update((draft) => {
      ...
      for (const item of draft.items) {
        item.sessionIds = item.sessionIds.filter(id => id !== sessionId) as typeof item.sessionIds
      }
    })
  }
}
