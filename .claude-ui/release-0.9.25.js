...
const OWNER = '891458249', REPO = 'Drafter', TAG = 'v0.9.25';
const DIST = 'D:\\ClaudeUI\\dist';
const ASSETS = [
  'DeskTopUI Setup 0.9.25.exe',
  'DeskTopUI Setup 0.9.25.exe.blockmap',
  'latest.yml',
];
...
(async () => {
  ...
  const notes = [
    '### v0.9.25(2026-08-10):导航交互三项优化',
    '',
    '- **滚轮不再误滚导航栏**:导航只吃悬停代理滚动',
    '- **回到底部按钮移到导航栏之下**(消息区右下角)',
    '- **macOS Dock 式边缘感应**:悬停感应区扩到整条导航;光标越过导航上/下边缘时,最上/最下一条保持完整放大显示,端点项更好选中',
    '',
  ...
  try {
    ...
    rel = await api('POST', `/repos/${OWNER}/${REPO}/releases`, { tag_name: TAG, name: 'DeskTopUI v0.9.25', body: notes, draft: false, prerelease: false });
